...
- **`paper/` is read-only — inspect, never change.** The manuscript is the user's own writing. Read it freely (cat/grep, and `git -C paper status|log|diff|show|blame`) to answer questions, debug a LaTeX build, or check what a section claims. Never create, edit, rename, move or delete anything inside `paper/` — new `.tex` files, `ref.bib` and build artifacts included — and never run a LaTeX build in the tree (copy to the scratchpad and build there). Since 2026-09-15 `paper/` is **its own git repo** (private Wisconsin GitLab `nuqu-paper`, branch `main`), still gitignored by the main repo, so main-repo `git status` never shows manuscript changes — use `git -C paper status`. **Git on `paper/` is read-only too:** no push, pull or fetch, and no commit, add, checkout, restore, reset, clean, stash, branch or tag. Draft replacement text in `docs/` or the scratchpad and tell the user what to paste in. A `PreToolUse` hook (`.claude/hooks/protect-paper.py`) enforces all of this; a block is the rule working, not a bug to route around.
...
- **Rigorous Fock-basis cutoff from Tong et al. 2022.** `estimate_boson_cutoff` (renamed from `calculate_fock_cutoff` on 2026-05-25; the shared per-site boson cutoff feeding both the Fock and NS-amplitude paths) in `src_PI/hamiltonians/core/EFTParameters.py` now has a `boson_cutoff_method` switch (a Config axis, threaded through the sweep): `'heuristic'` (default) is the hand-rolled `n_q = max(4, ceil(4 + log₂(1+A)))`; `'tong'` calls `classical/trimci/tong_bound.py::cutoff_predictions` and returns the certified `max(n_b_eng, n_b_spec1) = 4–5` (A-flat), a first-draft instantiation of the Tong bound (SCS occupation + spectral tails) that the classical nb/N_f study confirms is safe. The Tong–Albert–McClean–Preskill–Su 2022 paper (arXiv:2110.06942, PDF in `claude/research/bosonic-encodings/`) proves `N_f = O(polylog(1/ε))` for bosonic Hamiltonians with bounded-degree polynomial coupling (our case: H_AV degree 1, H_WT degree 2). **Still open (paper-grade):** the full Theorem-6 prefactor — bound the operator norms of the per-site polynomial pieces, plug the lattice geometry into their multi-site analysis, derive the explicit `N_f(ε, t, L, A, couplings)` formula. The `'tong'` switch makes the first-draft bound usable now; the rigorous prefactor derivation is the remaining publishable contribution.
