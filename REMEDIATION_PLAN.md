...
| **C1** — sparse RE | ✅ **GENUINELY COMPILED walk path (all 7 follow-up-audit exit gates, 2026-08-19).** `compiled_resources.compiled_walk_resources` + `sparse_oracle_mode='compiled'`: a real LCU walk cost where **every rotation is synthesized to Clifford+T** at the ΔE-derived precision (no "not-literal-T→Clifford" fallacy), **scalable primitives** (AddK shifts/folds, alias PREP, PauliLCU fermion — **no dense ≥2-qubit `MatrixGate`** at the counting boundary), the **full physical Hamiltonian incl. all 15 mixed H_AV/H_WT atoms** (mixed = Gilyén product BE_F⊗BE_b⊗BE_c via a rank-1 tensor factorisation of the two-mode boson factor), **precision-wired** (tighter ΔE → more T; Λ invariant), and the **reflection cost** included. **Walk_T = 2.25M @ L=2 dim=1 n_b=2** (all rotations synthesized), Λ=3823 (tighter, valid). Earlier rungs (kept for history): first build non-Hermitian (invalid walk) → Hermitian rebuild (`hermitian_boson_encoding`/`hermitian_bundle`, qubitizes) → first follow-up audit found the `hermitian_cost_model` was a hand `_t_complexity_` (still available, labelled optimistic) → **this genuinely compiled path.** **PauliLCU (C3) remains the publication anchor** (validated at every L; the compiled sparse path is the scaling vehicle once cluster-scaled). Scaling honesty: the matching-dilation *enumerates* the ≤2^{n_b} edges/atom — n_b=2 is the **fixed** cutoff, so O(1)/atom and **linear in atoms (∝ sites)**; the sparse *advantage* is the tighter Λ, not sub-linear n_b scaling. |
...
- **P0-1 single-mode atom: ✅ DONE.** `matching_dilation.py` compiles the full single-mode boson
  ...
  **Two-mode (H_WT) atoms DONE** via mode_c fold-conjugation (fold out the n_c step → flattened shift
  becomes a power of two → single-mode machinery); all four shapes (hopping/co-ladder, real+complex)
  qubitize. **QROM cost form: measured, NOT needed at n_b=2** (fixed ~2.4k T overhead loses to
  per-edge until n_b≈4-5). Covers the FULL boson sector.
- **P0-2 composite: ✅ DONE (full boson + fermion).** `compiled_bundle.py` assembles the compiled
  single- and two-mode boson atoms + fermion into a real `PREP·SELECT·PREP†` composite (heterogeneous
  dispatch); on small instances the **production circuit** gives `α_tot·⟨0|U|0⟩ = H`, qubitizes, and
  **fully decomposes to a genuine circuit-traversed T-count** (no `DecomposeNotImplementedError`, no
  hand `_t_complexity_`).
- **P0-3 equivalence tests: ✅ DONE** on the production composite (decomposes; block == H;
  Hermitian/self-inverse/qubitization phases; complex + two-mode).
- **P0-4 precision/error budget: ✅ DONE** (`precision_budget.py`). Every approximation parameter
  derived from the target ΔE + λ: `ε_QPE` (→ N_walk) + `ε_BE` = λ·(ε_prep + ε_rot) ≤ ΔE (ε_BE is a
  *systematic* perturbation, N_walk-independent). Wired into `estimate_hermitian_sparse_resources
  (delta_E=…)`: tightening ΔE raises Walk_T (166k→212k over 10→0.1 MeV) + qubits, λ precision-
  invariant. Surfaced that the old `1e-3` was too loose (implied ~4.5 MeV BE error; principled value
  5.6e-5).
- **Follow-up audit (2026-08-19) — all 7 exit gates addressed** (`codex_audit/c1_sparse_compiled_
  followup_2026-08-19.md`). The prior `compiled_bundle` cost still counted only *literal* T (charging
  rotations + dense ≥2-qubit `MatrixGate`s as free/Clifford), refused mixed atoms, and wasn't a real
  traversal. `compiled_resources.py` fixes all of it: (1)+(6) mixed atoms compiled + full physical
  Hamiltonian estimates; (2) scalable primitives (AddK/alias/PauliLCU, no dense `MatrixGate`); (3)
  every rotation synthesized to Clifford+T; (4) ΔE→precision→traversal; (5) `sparse_oracle_mode=
  'compiled'` config mode (compiler_derived=True); (7) resource test asserts 0 dense ≥2-qubit gates
  at the boundary. **Walk_T = 2.25M** (synthesized). `test_compiled_resources.py` (7 tests).
- **Remaining (scale, not correctness):** the enumerated per-edge form is O(1)/atom at the fixed
  n_b=2 and linear in atoms; the large-L bundle runs on the cluster. The verification-path
  `compiled_bundle` (dense PREP/fermion, `cirq.unitary`) stays as the exact block-correctness check.
